...
{
    let log = FileRecordLog::At(Scratch("order")).expect("At creates the log file and its directory");
...
{
    let log = FileRecordLog::At(Scratch("empty")).expect("At creates the log file and its directory");
